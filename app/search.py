...
class Searcher:
    ...
    def _embed_query(self, query: str) -> tuple[float, ...]:
        """Embed a normalized query once and reuse it across repeated calls.

        ONNX inference dominates request latency on CPU-only machines. A small
        bounded cache keeps the API's warm-query path fast without retaining
        an unbounded amount of user input. Tuple makes the cached value
        immutable; callers convert it to the list expected by Qdrant.
        """
        assert self.embedder is not None
        normalized = " ".join(query.split())
        return tuple(float(v) for v in next(self.embedder.embed([normalized])))
